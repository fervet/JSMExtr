...
functionMetricsK <- 70;
functionMetricsAlpha <- 0.9;
```

```{r, echo=FALSE}
# Inform chosen arguments
cat("TCLUST arguments: k=", functionMetricsK, ", alpha=", functionMetricsAlpha)
```
...
functionClusters <- tclust (jsMetricsColumnsOnly, k = functionMetricsK, alpha = functionMetricsAlpha)
```
...
```{r}
write.table(allClustersCsv, file = "allClusters.csv", sep = ",", col.names = NA, qmethod = "double")
```
